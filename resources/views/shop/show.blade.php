...
@section('content')
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-2">
      {{-- Image --}}
      <div
        class="aspect-square overflow-hidden rounded-3xl border border-gray-100 bg-gradient-to-br from-gray-50 to-gray-100 shadow-sm">
        @if ($product->image)
      ...
      <div class="flex flex-col justify-center">
        <span
          class="inline-flex w-fit items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">
          {{ $product->category }}
        </span>
        <h1 class="mt-4 text-3xl font-bold text-gray-900 sm:text-4xl">{{ $product->name }}</h1>

        <div class="mt-4 flex items-center gap-3">
          <span class="text-3xl font-extrabold text-indigo-600"><span class="text-sm">Rp</span>
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center rounded-xl border border-gray-200 bg-gray-50">
                ...
                <input type="number" name="quantity" id="quantity" value="1" min="1"
                  max="{{ $product->stock }}"
                  class="w-16 border-0 bg-transparent text-center text-sm font-semibold text-gray-800 focus:outline-none focus:ring-0">
              ...
              <button type="submit"
                class="flex-1 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-200 transition-all hover:shadow-xl hover:shadow-indigo-300 hover:-translate-y-0.5 active:scale-95">
                Add to Cart
              </button>
            </div>
          </form>
        @else
          <div class="mt-8">
            <button disabled
              class="w-full rounded-2xl bg-gray-200 px-8 py-3.5 text-sm font-bold text-gray-400 cursor-not-allowed">
              Out of Stock
            </button>
          </div>
        @endif
        ...
        <div class="mt-8 grid grid-cols-3 gap-4 border-t border-gray-100 pt-8">
          <div class="text-center">
            <svg class="mx-auto h-6 w-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
