  ...
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mt-6 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
      <form action="{{ route('shop.index') }}" method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <select name="category"
          class="rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-2.5 text-sm text-gray-700 transition-all focus:border-indigo-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100">
        ...
        <div class="flex gap-2">
          <button type="submit"
            class="rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-200 transition-all hover:bg-indigo-700 hover:shadow-lg">
            Filter
          </button>
          @if (request()->hasAny(['search', 'category', 'sort']))
            <a href="{{ route('shop.index') }}"
              class="rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-200">
              Clear
            </a>
          @endif
        </div>
    ...
    @if ($products->isEmpty())
      <div class="mt-16 text-center">
        ...
        <a href="{{ route('shop.index') }}"
          class="mt-6 inline-flex rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-indigo-700">
          Clear Filters
        </a>
      </div>
    @else
